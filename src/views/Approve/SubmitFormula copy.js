...
export default function SubmitFormula() {
  ...
  const columns = [
    {
      ...
      sortable: true,
      cell: row => (<div>{row.product_id}</div>)
    },
    {
      name: 'Tên sản phẩm',
  ...
  const onRowClicked = (row) => {
    ...
    history.push("/submit/formula", { product_info: row.product_id });
  }
      ...
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader color="info" className={classes.cardTitleWhite} >
            Sản Phẩm Đang Tham Gia Phát Triển Công Thức
          </CardHeader>
